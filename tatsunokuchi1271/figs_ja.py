...
# 辰刻 bands (定時法, 地方真太陽時)
kokus=[("亥",21),("子",23),("丑",1),("寅",3),("卯",5)]
for i,(n,s) in enumerate(kokus):
    a=(s-LATOFF*1.0); a=a-24 if a>12 else a
    b=a+2
    ax.axvspan(a,b,color=("#f2f2f7" if i%2 else "#e6e9f2"),zorder=0)
    ax.text(max((a+b)/2,-1.6),78,f"{n}の刻",ha="center",va="center",fontsize=10,color="#333")
for x,lab in [(4+32/60,"天文薄明開始 04:32"),(5+57/60,"日出 05:57")]:
    ax.axvline(x,color="#888",ls=":",lw=1); ax.text(x+0.05,-26,lab,rotation=90,fontsize=7.5,color="#555",va="bottom")
ax.plot(hs,alt_moon,color=C["moon"],lw=2,label="月（月齢12.0、満ちていく凸月）")
ax.plot(hs,alt_vj,color=C["vj"],lw=2.4,label="Vela Jr.（RX J0852.0−4622）")
ax.plot(hs,alt_ori,color=C["ori"],lw=1.6,ls="--",label="オリオン座流星群 放射点")
ax.plot(hs,alt_sta,color=C["sta"],lw=1.6,ls="--",label="おうし座南流星群 放射点")
ax.plot(hs,np.where(alt_sun<-30,np.nan,alt_sun),color=C["sun"],lw=1.2,ls="-.",label="太陽")
ax.axhline(0,color="k",lw=0.8)
ax.annotate("月没 03:37",xy=(3+37/60,0),xytext=(2.2,-18),arrowprops=dict(arrowstyle="->",color=C["moon"]),fontsize=8.5,color="#8a6500")
ax.annotate("Vela Jr. 出 02:43",xy=(2+43/60,0),xytext=(1.2,-27),arrowprops=dict(arrowstyle="->",color=C["vj"]),fontsize=8.5,color=C["vj"])
ax.annotate("南中 05:57\n高度 10.9°",xy=(5+57/60,10.9),xytext=(4.6,22),arrowprops=dict(arrowstyle="->",color=C["vj"]),fontsize=8.5,color=C["vj"])
ax.set_xlim(-2,6.5); ax.set_ylim(-30,85)
ax.set_xticks(range(-2,7)); ax.set_xticklabels([f"{h%24:02d}:00" for h in range(-2,7)])
ax.set_xlabel("日本標準時 JST（1271年10月17日夜 → 18日朝, ユリウス暦）"); ax.set_ylabel("高度 [°]")

ax.legend(loc="upper left",fontsize=8,framealpha=0.9,bbox_to_anchor=(0.0,0.93))
ax.grid(alpha=0.3)
fig.tight_layout(); fig.savefig(_setup.path("figures/ja","fig1_altitude.png")); plt.close(fig)
...
def gc_path(raz,ralt,paz,palt,ext=40,n=60):
    ...
    return np.array(out)
DIR={0:"北(子)",45:"北東(丑寅)",90:"東(卯)",135:"南東(辰巳)",180:"南(午)",225:"南西(未申)",270:"西(酉)",315:"北西(戌亥)",360:"北(子)"}
def panorama(h,fname,title,meteors=False):
    ...
    for c in cons:
        ...
        if -8<b[0]<72 and az0<a[0]<az1: ax.text(a[0],b[0],c["name_ja"],color="#8fb0e0",fontsize=6.5,ha="center",zorder=2)
    # Vela Jr
    ...
    ax.axvline(212,color="#ffb000",ls="--",lw=1); ax.text(213,62,"江ノ島\n(Az=212°, 1.6 km)",color="#ffb000",fontsize=8)
    if meteors:
    ...
    ax.set_ylabel("高度 [°]"); ax.set_title(title,fontsize=10.5)
    fig.tight_layout(); fig.savefig(_setup.path("figures/ja",fname)); plt.close(fig)
panorama(2.0,"fig2_sky0200.png","1271年10月18日 02:00 JST（地方真太陽時 02:33）龍口寺から見た東〜南〜西の空")
panorama(3+45/60,"fig3_sky0345.png","1271年10月18日 03:45 JST（月没後・寅の刻, 地方真太陽時 04:18）龍口寺から見た東〜南〜西の空")
...
ray(212,"#d08a00","江ノ島 212°")
ray(135,"#c2185b","辰巳 135°",ls="--"); ray(315,"#c2185b","戌亥 315°",ls="--")
ray(147.2,C["vj"],"Vela Jr. 出 147°",r=0.8)
ray(269.4,"#b08000","月没 269°",r=0.8)

fig.tight_layout(); fig.savefig(_setup.path("figures/ja","fig4_compass.png")); plt.close(fig)
...
for code in ("ORI","STA"):
    ...
    ax.text(X[0]+0.04,Y[0]+0.05,{"ORI":"オリオン座群\n放射点","STA":"おうし座南群\n放射点"}[code],color=cols[code],fontsize=8,zorder=6)
...
ax.text(x0+0.02,y0-0.1,"辰巳→戌亥\n（記述の進行方向）",color="#ff8ccf",fontsize=8)
x,y=xy(212,0); ax.plot([0,x],[0,y],color="#ffb000",lw=0.9,ls="--"); ax.text(x*0.8+0.03,y*0.8,"江ノ島 212°",color="#ffb000",fontsize=8)
sat=observe_planet(eph,"saturn",t,site); X,Y=xy(float(sat.az),float(sat.alt)); ax.scatter([X],[Y],s=55,c="#f2d9a0",edgecolors="#a07830",lw=0.8,zorder=6); ax.text(X[0] if hasattr(X,"__len__") else X,(Y[0] if hasattr(Y,"__len__") else Y)+0.04,"土星",color="#f2d9a0",fontsize=8,ha="center",zorder=6)
...
fig.tight_layout(); fig.savefig(_setup.path("figures/ja","fig5_allsky_meteors.png")); plt.close(fig)
